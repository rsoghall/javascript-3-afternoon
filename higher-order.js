...
//Code Here
let myStrongest = monstersInYourPocket.filter((element, index, wholeArray) => element ['CP'] > 200);
...
//let ordersTotal = orders.map(orders => orders.price += orders.price * orders.tax);
let orderTotals = [];

//orders.map((price, tax) => order.price += order.price * order.tax);
